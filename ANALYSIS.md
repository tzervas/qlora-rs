...
- **Edition:** Workspace-managed
- **Key Dependencies:**
  - `candle-core`, `candle-nn` - ML tensor operations
  - `peft-rs` - LoRA adapter management (local workspace dependency at `../peft-rs`)
...
- **GGUF Support:** Commented out placeholder (`# gguf-rs = "0.1"`)
...
**Missing/TODO:**
- ⚠️ Double quantization NOT implemented (config exists but unused)
- ⚠️ `zero_points` field exists but never used (asymmetric quantization not implemented)
...
**Missing/Critical TODOs:**
- ❌ `merge_and_export_gguf()` - NOT IMPLEMENTED
  - Returns explicit error: "merge_and_export not yet implemented"
  - Needed for deployment workflows
- ❌ GGUF integration library not available
  - Commented out `# gguf-rs = "0.1"`
  - Current GGUF writing is manual/incomplete
- ⚠️ Tensor offset calculation hardcoded to 0
  - File format likely incorrect for actual llama.cpp compatibility
- ⚠️ No metadata/KV pairs support
  - Real GGUF files need model metadata
...
1. **GGUF Export**
   ...
   - Tensor offset calculation incomplete
   - No metadata/KV pairs
   - No model merge functionality
