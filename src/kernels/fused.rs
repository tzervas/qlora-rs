...
#[cube(launch)]
pub fn fused_nf4_matmul_kernel<F: Float>(
    x: &Array<F>,
    w_packed: &Array<u32>,
    scales: &Array<F>,
    out: &mut Array<F>,
    ...
    for k_tile in 0u32..num_k_tiles {
        ...
        if row < m && col < n {
            ...
            for i in 0u32..TILE_SIZE {
                acc = acc + x_tile[UNIT_POS_Y * TILE_SIZE + i] * w_tile[i * TILE_SIZE + UNIT_POS_X];
            }
        }
...
#[cube(launch)]
pub fn fused_nf4_batched_matmul_kernel<F: Float>(
    x: &Array<F>,
    w_packed: &Array<u32>,
    scales: &Array<F>,
    out: &mut Array<F>,
    ...
    if row < m && col < n {
        out[out_batch_offset + row * n + col] = acc;
    }
}
...
/// - r: LoRA rank
#[cube(launch)]
pub fn fused_nf4_lora_forward_kernel<F: Float>(
    x: &Array<F>,
    w_packed: &Array<u32>,
    w_scales: &Array<F>,
    lora_a: &Array<F>,
    ...
    for i in 0u32..in_dim {
        ...
        let w_val = nf4_val * w_scales[scale_idx];

        base_acc = base_acc + x[row * in_dim + i] * w_val;
    }
    ...
    for r in 0u32..rank {
        ...
        for i in 0u32..in_dim {
            hidden = hidden + x[row * in_dim + i] * lora_a[i * rank + r];
        }
        // Multiply by B[r, col]
        lora_out = lora_out + hidden * lora_b[r * out_dim + col];
    }

    // Combine: base + lora (scaling is applied externally or via alpha param)
    out[row * out_dim + col] = base_acc + lora_out;
}
...
#[cube(launch)]
pub fn fused_nf4_matmul_bias_act_kernel<F: Float>(
    x: &Array<F>,
    w_packed: &Array<u32>,
    scales: &Array<F>,
    bias: &Array<F>,
...
#[cube(launch)]
pub fn fused_nf4_add_kernel<F: Float>(
    w_packed: &Array<u32>,
    scales: &Array<F>,
    x: &Array<F>,
    out: &mut Array<F>,
...
#[cube(launch)]
pub fn simple_nf4_matmul_kernel<F: Float>(
    x: &Array<F>,
    w_packed: &Array<u32>,
    scales: &Array<F>,
    out: &mut Array<F>,
