...
fn test_standard_optimizer_trains_weights() {
    ...
    assert!(result.is_ok(), "init_optimizer should succeed with VarBuilder-created layers: {:?}", result.err());
}
...
fn test_paged_optimizer_memory_tracking() {
    ...
    assert!(usage_after_param1 > 0, "GPU usage should increase after paging");
    assert!(state.is_gpu_resident("param1"), "param1 should be GPU resident");
    
    // Page param2 to GPU
    let _ = state.page_to_device("param2", &device).unwrap();
    let usage_after_param2 = state.current_gpu_usage;
    assert!(usage_after_param2 > usage_after_param1, "GPU usage should increase further");
    assert!(state.is_gpu_resident("param2"), "param2 should be GPU resident");
    ...
    assert!(usage_after_cpu < usage_after_param2, "GPU usage should decrease after paging to CPU");
    assert!(!state.is_gpu_resident("param1"), "param1 should no longer be GPU resident");
}
...
fn test_paged_optimizer_performs_updates() {
    ...
    optimizer.step_param("test_param", &mut param_clone, &grad).unwrap();
...
fn test_lru_order_tracking() {
    ...
    assert_eq!(state.gpu_resident_count(), 3, "All 3 params should be GPU resident");
    
    // Access 'c' again to make it most recent
    let _ = state.page_to_device("c", &device).unwrap();
    
    // Still all three should be GPU resident
    assert_eq!(state.gpu_resident_count(), 3, "All 3 params should still be GPU resident");
}
...
fn test_trainer_with_paged_optimizer() {
    ...
    let mut trainer = trainer;
    let result = trainer.init_optimizer(&layers);
    assert!(result.is_ok(), "Paged optimizer init should succeed: {:?}", result.err());
    
    // Check memory stats are available
    let stats = trainer.optimizer_memory_stats();
    assert!(stats.is_some(), "Memory stats should be available for paged optimizer");
}
...
fn test_init_optimizer_fails_without_varbuilder() {
    ...
    let result = trainer.init_optimizer(&layers);
    assert!(result.is_err(), "init_optimizer should fail without VarBuilder-created layers");
}
